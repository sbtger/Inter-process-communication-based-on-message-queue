...
int main(void)
{
	...
	char buf1[128];
	sprintf(buf1,"kill %d",pid1);
	msg_mbuf.mtype = 10;

	memcpy(msg_mbuf.mtext,buf1,sizeof(buf1));		/*复制字符串*/
	ret = msgsnd(msg_id, &msg_mbuf, sizeof(buf1), msg_sflags);/*发送消息*/
	if( -1 == ret)
		printf("发送bash命令失败\n");	
	msg_info.msg_lspid = pid1;
